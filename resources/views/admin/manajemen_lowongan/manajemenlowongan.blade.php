...
@section('content')
    <div class="container mx-auto px-6 py-8">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            ...
            <div class="bg-[#1A73E8] rounded-xl shadow-lg p-6 hover:shadow-xl transition-shadow duration-300 flex flex-col">
                <div class="flex items-start mb-4">
                    <div class="w-12 h-12 bg-blue-100 rounded-full flex items-center justify-center mr-4">
                        @if($vacancy->job_logo)
                            <img src="{{ asset('storage/' . $vacancy->job_logo) }}" alt="Logo Perusahaan" class="w-[30px] h-[30px] object-contain rounded-lg">
                        @else
                    ...
                    <div class="flex-grow"> {{-- Tambahkan flex-grow agar konten mengambil ruang yang tersedia --}}
                        <div class="flex justify-between items-center w-full">
                            ...
                            {{-- ID Lowongan di sini --}}
                            <div class="w-8 h-8 rounded-full bg-white flex items-center justify-center text-[#1A73E8] text-sm font-bold ml-2">
                                {{ $vacancy->id }}
                            </div>
                        </div>
                ...
                <div class="flex flex-wrap gap-2 mb-4">
                    ...
                    <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-700">
                        Rp {{ number_format($vacancy->min_salary, 0, ',', '.') }} - Rp {{ number_format($vacancy->max_salary, 0, ',', '.') }}/bulan
                    </span>
                    @endif
                </div>
                ...
                <div class="flex gap-2 mt-auto">
                    ...
                    <button type="button"
                            ...
                            onclick="showJobDetail({{ $vacancy->id }})">
                        Lihat
                    </button>
                    {{-- Tombol Hapus --}}
                    <form action="{{ route('admin.manajemen.lowongan.destroy', $vacancy) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus lowongan ini?');">
                        @csrf
                        @method('DELETE')
                        <button type="submit"
    ...
    async function showJobDetail(jobId) {
        try {
            ...
            if (jobData.min_salary && jobData.max_salary) {
                document.getElementById('modalJobSalary').textContent = 'Gaji: Rp ' + new Intl.NumberFormat('id-ID').format(jobData.min_salary) + ' - Rp ' + new Intl.NumberFormat('id-ID').format(jobData.max_salary) + '/bulan';
            } else {
