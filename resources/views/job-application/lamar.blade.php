...
<div class="min-h-screen bg-gray-100 flex justify-center items-start p-5">
    <div class="w-full max-w-md bg-white rounded-lg overflow-hidden shadow-lg">
        ...
        <div class="bg-[#FFA98F] p-5 text-white flex items-center justify-center gap-4">
            <img src="{{ asset('asset/page/lamar.png') }}" alt="Illustrasi Orang Bergabung" class="w-20 h-auto object-contain">
        ...
        <div class="p-6 pb-8">
            ...
            <form action="{{ route('lamar.store') }}" method="POST" enctype="multipart/form-data" id="jobApplicationForm">
                @csrf

                <!-- Position Selection -->
                <div class="floating-label-input">
                    <select name="position_id" id="posisi" required>
                ...
                <div class="floating-label-input">
                    <input type="text" name="full_name" id="nama" placeholder="Nama Lengkap" value="{{ old('full_name') }}" required>
                    <label for="nama">Nama Lengkap</label>
                </div>

                <div class="floating-label-input">
                    <input type="tel" name="phone_number" id="nomor" placeholder="Nomor" value="{{ old('phone_number') }}" required>
                ...
                <div class="floating-label-input">
                    <input type="text" name="major" id="jurusan" placeholder="Jurusan" value="{{ old('major') }}" required>
                ...
                <div class="floating-label-input">
                    <select name="experience_level" id="pengalaman" required>
                        <option value="" disabled selected></option>
                        <option value="Fresh Graduate" {{ old('experience_level') == 'Fresh Graduate' ? 'selected' : '' }}>Fresh Graduate</option>
                        <option value="1-2 Tahun" {{ old('experience_level') == '1-2 Tahun' ? 'selected' : '' }}>1-2 Tahun</option>
                        <option value="3-5 Tahun" {{ old('experience_level') == '3-5 Tahun' ? 'selected' : '' }}>3-5 Tahun</option>
                        <option value="5+ Tahun" {{ old('experience_level') == '5+ Tahun' ? 'selected' : '' }}>5+ Tahun</option>
                    </select>
                ...
                <div class="mb-5">
                    ...
                    <input type="file" name="cv_file" id="cv-upload" accept=".pdf" style="display: none;" required>
                </div>
                ...
                <div class="mb-5">
                    ...
                    <label for="portfolio-upload" class="upload-btn">
                        <div class="upload-icon">+</div>
                        <span class="upload-text" id="portfolio-text">Unggah Portfolio Dalam Bentuk Pdf (Opsional)</span>
                    </label>
                    <input type="file" name="portfolio_file" id="portfolio-upload" accept=".pdf" style="display: none;">
                </div>
...
<script>
    ...
    async function fetchPositions() {
        try {
            const response = await fetch('{{ route("lamar.positions") }}');
            if (!response.ok) {
                throw new Error(`HTTP error! status: ${response.status}`);
            }
            const positions = await response.json();
            populateDropdown(posisiSelect, positions, "Pilih Posisi");
        } catch (error) {
            console.error('Gagal mengambil data posisi:', error);
            alert('Gagal memuat daftar posisi. Silakan coba lagi nanti.');
        }
    }
    ...
    posisiSelect.addEventListener('change', function() {
        if (this.value !== "") {
            this.classList.add('valid');
            positionNameInput.value = this.options[this.selectedIndex].text;
        } else {
            this.classList.remove('valid');
    ...
    document.getElementById('portfolio-upload').addEventListener('change', function() {
        if (this.files.length > 0) {
            ...
            document.getElementById('portfolio-text').textContent = 'Unggah Portfolio Dalam Bentuk Pdf (Opsional)';
        }
    });
