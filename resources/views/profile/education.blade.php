                    ...
                    @forelse ($educations as $edu)
                        <div class="flex items-start justify-between p-4 border rounded-xl shadow-sm hover:border-orange-300">
                            ...
                            <div class="flex items-center space-x-2">
                                <a href="{{ route('profile.education.edit', $edu) }}" title="Edit" class="text-gray-400 hover:text-blue-500 p-1 rounded-full hover:bg-blue-50">
                                ...
                                <form action="{{ route('profile.education.delete', $edu) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus riwayat pendidikan ini?');">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" title="Hapus" class="text-gray-400 hover:text-red-500 p-1 rounded-full hover:bg-red-50">
                ...
                <form action="{{ route('profile.education.store') }}" method="POST">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                        <div class="md:col-span-2">
                            ...
                            <input type="text" name="university_name" id="university_name" value="{{ old('university_name')}}" class="w-full border border-gray-300 rounded-lg p-2.5" placeholder="Contoh: Universitas Sebelas Maret">
                        ...
                        <div>
                            ...
                            <input type="text" name="degree" id="degree" value="{{ old('degree') }}" class="w-full border border-gray-300 rounded-lg p-2.5" placeholder="Contoh: Sarjana Komputer">
                        ...
                        <div>
                            ...
                            <input type="text" name="major" id="major" value="{{ old('major') }}" class="w-full border border-gray-300 rounded-lg p-2.5" placeholder="Contoh: Ilmu Komputer">
                        ...
                        <div>
                            ...
                            <input type="text" name="country" id="country" value="{{ old('country') }}" class="w-full border border-gray-300 rounded-lg p-2.5" placeholder="Contoh: Indonesia">
                            @error('country')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                        </div>
                        <div>
                            <label for="city" class="block text-sm font-medium text-gray-600 mb-1">Kota</label>
                            <input type="text" name="city" id="city" value="{{ old('city') }}" class="w-full border border-gray-300 rounded-lg p-2.5" placeholder="Contoh: Yogyakarta">
                        ...
                        <div>
                            ...
                            <div class="mt-3">
                                <label for="currently_studying" class="flex items-center cursor-pointer">
                                    <input type="checkbox" name="currently_studying" id="currently_studying" value="1" {{ old('currently_studying') ? 'checked' : '' }} class="h-4 w-4 rounded text-orange-500">
                        ...
                        <div class="md:col-span-1">
                           ...
                           <input type="text" name="ipk" id="ipk" value="{{ old('ipk') }}" class="w-full md:w-1/2 border border-gray-300 rounded-lg p-2.5" placeholder="Contoh: 3.75">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea name="description" id="description" rows="5" class="w-full border border-gray-300 rounded-lg p-2.5 resize-y" placeholder="Jelaskan kegiatan relevan, prestasi, atau informasi lain (opsional)">{{ old('description') }}</textarea>
                            @error('description')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                        </div>
                    </div>
                    <div class="mt-10 flex justify-end gap-4">
                        <button type="reset" class="py-2.5 px-8 rounded-lg font-semibold border border-gray-400 text-gray-600 hover:bg-gray-100">Batal</button>
                        <button type="submit" class="py-2.5 px-8 rounded-lg font-semibold bg-orange-500 text-white hover:bg-orange-600">Simpan</button>
                    </div>
                </form>
            </main>
        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function toggleEndDateFields() {
            ...
            endMonthSelect.disabled = isDisabled; endYearSelect.disabled = isDisabled;
            if(isDisabled) {
